...
function AuthPageContent() {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (loginRole === 'super-admin') {
        const isOverseer = loginEmail.toLowerCase() === 'admin@takwinpro.dz' || loginEmail.toLowerCase() === 'overseer@takwinpro.dz';
        if (!isOverseer) {
          setErrorMsg(language === 'ar' ? 'البريد الإلكتروني غير مصرح به كمسؤول المنصة.' : 'Email non autorisé en tant que super administrateur.');
          setLoading(false);
          return;
        }
        localStorage.setItem('takwin_session_role', 'super-admin');
        localStorage.setItem('takwin_session_email', loginEmail);
        setSuccessMsg(t('authSuccess'));
        setTimeout(() => {
          router.push('/super-admin');
        }, 1000);
      } else if (loginRole === 'company') {
        const company = await db.getCompanyByEmail(loginEmail);
        if (!company) {
          setErrorMsg(language === 'ar' ? 'مؤسسة غير مسجلة بهذا البريد الإلكتروني.' : 'Aucune institution enregistrée avec cet e-mail.');
          setLoading(false);
          return;
        }
        localStorage.setItem('takwin_session_role', 'company');
        localStorage.setItem('takwin_session_email', loginEmail);
        localStorage.setItem('takwin_session_company_id', company.id);
        setSuccessMsg(t('authSuccess'));
        setTimeout(() => {
          router.push('/company');
        }, 1000);
      } else {
        const trainee = await db.getTraineeByEmail(loginEmail);
        if (!trainee) {
          setErrorMsg(language === 'ar' ? 'موظف غير مسجل بهذا البريد الإلكتروني.' : 'Aucun agent enregistré avec cet e-mail.');
          setLoading(false);
          return;
        }
        localStorage.setItem('takwin_session_role', 'trainee');
        localStorage.setItem('takwin_session_email', loginEmail);
        localStorage.setItem('takwin_session_trainee_id', trainee.id);
        localStorage.setItem('takwin_session_company_id', trainee.companyId);
        setSuccessMsg(t('authSuccess'));
        setTimeout(() => {
          router.push('/trainee');
        }, 1000);
      ...
      setLoading(false);
      setErrorMsg(language === 'ar' ? 'حدث خطأ أثناء الاتصال بقاعدة البيانات.' : 'Erreur lors de la connexion à la base de données.');
      console.error(err);
    }
  };
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Add corporate entity to database wrapper
      await db.addCompany(regCompanyName, regSector, regEmail);
      setLoading(false);
      setSuccessMsg(
        language === 'ar' 
          ? 'تم تسجيل مؤسستك بنجاح! يمكنك الآن تسجيل الدخول.' 
          : 'Votre institution a été inscrite avec succès ! Vous pouvez maintenant vous connecter.'
      );
      ...
      // Auto toggle to login tab after success
      setTimeout(() => {
        setActiveTab('login');
