...
import { db, Trainee, SupportTicket, OFFICIAL_TRACKS } from '@/lib/db';
import { 
  BookOpen, 
  GraduationCap, 
  Clock, 
...
export default function TraineePortalPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const list = await db.getTrainees();
        setTrainees(list);
        if (list.length > 0) {
          // Default to first trainee
          setSelectedTraineeId(list[0].id);
          setCurrentTrainee(list[0]);
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  // Update current trainee view when selector changes
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-[#fbf8f3] text-[#2d2621]">
      ...
      <main className="flex-grow p-6 sm:p-8 lg:p-12 overflow-y-auto mb-16 lg:mb-0">
        ...
        {/* Dynamic Trainee Account Selector for Demo Testing */}
        <div className="bg-[#F3E4C9] p-4 rounded-xl mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <User className="w-5 h-5 text-[#3E5C46]" />
            <span className="text-xs font-bold text-[#3E5C46]">
              {language === 'ar' ? 'اختر حساب موظف للتجريب الإيضاحي:' : 'Sélectionnez un profil agent pour le test :'}
            </span>
          </div>
          ...
          <select
            ...
            className="underline-input text-xs font-bold text-[#3E5C46] bg-transparent py-1.5 max-w-xs"
          >
            {trainees.map(t => (
              <option key={t.id} value={t.id} className="text-[#3E5C46] bg-[#fbf8f3] font-bold text-xs">
                {t.name} ({t.progress}%) - {t.companyName}
              </option>
            ))}
          </select>
        </div>
              ...
              <section className="mb-12 animate-fadeIn">
                <div className="flex items-center justify-between border-b border-[#F3E4C9] pb-3 mb-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <a
                      href={`/verify?code=CERT-2026-${(currentTrainee.id).toUpperCase()}`}
                      target="_blank"
                      rel="noreferrer"
                  ...
                  <div className="grid grid-cols-2 gap-6 text-center border-b-2 border-[#3E5C46]/20 pb-6 z-10 relative">
                    ...
                    <div className="text-left font-mono text-[9px] sm:text-[10px] text-[#3E5C46] flex flex-col gap-0.5">
                      ...
                      <span className="font-bold text-[9px] text-[#3E5C46] mt-2 block font-sans">
                        REG. N°: CERT-2026-{(currentTrainee.id).toUpperCase()}
                      </span>
                    </div>
                    ...
                    <div className="text-right font-tajawal text-[10px] sm:text-[11px] text-[#3E5C46] flex flex-col gap-0.5">
                      ...
                      <span className="font-extrabold text-[9px] sm:text-[10px] text-[#3E5C46] mt-2 block font-mono">
                        رقم القيد: CERT-2026-{(currentTrainee.id).toUpperCase()}
                      </span>
                    </div>
                  </div>
