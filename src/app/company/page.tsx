...
import { db, Trainee, TrainingTrack, SupportTicket, CourseEvaluation, Lesson } from '@/lib/db';
import { 
  BookOpen, 
  Users2, 
  Settings2, 
...
export default function CompanyAdminPage() {
  ...
  const loadData = useCallback(async (activeCompanyId: string = companyId) => {
    setLoading(true);
    try {
      // Get company meta
  ...
  useEffect(() => {
    const role = localStorage.getItem('takwin_session_role');
    const storedCompanyId = localStorage.getItem('takwin_session_company_id');
    const searchParams = new URLSearchParams(window.location.search);
    const queryCompanyId = searchParams.get('companyId');
    
    setIsSuperAdmin(role === 'super-admin');
    
    let activeCompanyId = storedCompanyId || 'comp-2';
    
    if (role === 'super-admin' && queryCompanyId) {
      activeCompanyId = queryCompanyId;
    } else if (!role && !storedCompanyId) {
      router.push('/auth');
      return;
    }
